...
void NewImageUploadedEmailGenerator::run(int project_id)
{
    ...
    if (error.compare("") == 0) {
        ...
        foreach(QString admin, admins) {
            UserDao::queue_email(db, 0, admin.trimmed(), settings.get("site.name") + ": Project Image Uploaded [Project - " + projectTitle + "]", QString::fromUtf8(email_body.c_str()));
        }
