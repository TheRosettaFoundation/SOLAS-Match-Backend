...
static void UserTaskDeadlineEmailGenerator::run(int task_id, int translator_id)
{
    ...
    if(!user || !task) {
        ...
        qDebug() << "EmailGenerator: Generating UserClaimedTaskDeadlinePassed email FAILED, translator_id: " << QString::number(translator_id) << ", task_id: "  << QString::number(task_id);
    }

    if(error.compare("") == 0) {
        qDebug() << "EmailGenerator: Generating UserClaimedTaskDeadlinePassed email, translator_id: " << QString::number(translator_id) << " " << QString::fromStdString(user->email()) << ", task_id: "  << QString::number(task_id);
...
//else  qDebug() << "UserClaimedTaskMiddle send warning:" << QString::number(task->id());//(**)
        ...
        if (sendMessage) {
            UserDao::queue_email(db, translator_id, QString::fromStdString(user->email()), settings.get("site.name") + ": Task Deadline Passed", QString::fromUtf8(email_body.c_str()), LOW);
            UserDao::log_email_sent(db, translator_id, task_id, task->projectid(), 0, 0, 0, 0, "deadline_reminder_now_to_volunteer");
        }
    } else {
