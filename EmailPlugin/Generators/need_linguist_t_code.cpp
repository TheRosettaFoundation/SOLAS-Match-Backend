    ...
    if (error == "") {
        ...
        foreach (int admin_id, admin_ids) {
            ...
            if (!admin.isNull()) {
                ...
                UserDao::queue_email(db, admin_id, QString::fromStdString(admin->email()), "Need Sun Linguist T-Code for " + claimant->display_name(), QString::fromUtf8(email_body.c_str()), LOW);
